...
public class PageRankCentralityMetricWithGremlinService implements PageRankCentralityMetricService {
  ...
  public Double getValueFor(Resource resource) {
    Optional<CentralityMetricResult> optionalDegreeValue = centralityMetricStoreService
        .findById(CentralityMetricKey.of(PAGE_RANK_PROP_NAME, resource));
  ...
  public void compute() {
    ...
    centralityMetricStoreService.saveAll(gremlinService.traversal().withComputer().V().pageRank()
        .group()
        ...
        .map(e -> new CentralityMetricResult(
            CentralityMetricKey.of(PAGE_RANK_PROP_NAME, (String) e.getKey()),
