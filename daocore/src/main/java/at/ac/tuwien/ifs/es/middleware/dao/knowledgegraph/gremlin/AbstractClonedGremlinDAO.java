...
public abstract class AbstractClonedGremlinDAO implements SPARQLSyncingGremlinDAO {
    ...
    private class GraphConstruction implements Runnable {
            ...
            try {
                ...
                do {
                    ...
                    values = sparqlDAO.<SelectQueryResult>query(
                            ...
                            .value();
                    logger.trace("Fetch resources of knowledge graph. Offset: {}", offset);
                    if (values != null) {
                        for (Map<String, RDFTerm> row : values) {
                            BlankNodeOrIRI node = (IRI) row.get("resource");
                            String sIRI = BlankOrIRIJsonUtil.stringValue(node);

                            if (sIRI.equals("http://purl.org/ontology/mo/MusicArtist")) {
                                logger.debug("> Hit <http://purl.org/ontology/mo/MusicArtist>");
                            }
        ...
        public void run() {
            ...
            try (PrintWriter outWriter = new PrintWriter(new FileOutputStream(new File("out_resources.txt")))) {
                for (Map.Entry<BlankNodeOrIRI, Vertex> entry : vertexMap.entrySet()) {
